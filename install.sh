...
if [ ! -n "$BASH" ]; then
  ...
  bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://raw.githubusercontent.com/Jaxer159/friendly-telegram/master/install.sh) '"$*"
  exit $?
fi
...
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/Jaxer159/friendly-telegram || { errorout "Clone failed."; exit 3; }
